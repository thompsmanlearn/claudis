...
B-059: agent_feedback write-back schema + close-session rule (COMPLETE 2026-04-26)

Executed inline during the 2026-04-26-1323 session without a formal card. Added action_summary, action_session, action_result_url columns to agent_feedback (nullable text). Updated close-session.md v27: every processed=true UPDATE must include action_summary and action_session; "Deferred: [reason]" prefix is the required form for items not acted on. No LEAN_BOOT.md changes (defer logic belongs entirely at close). Commit 3235128.
