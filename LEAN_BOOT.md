...
1. `git pull` on `~/aadp/claudis/`. If pull fails, proceed on local state and notify Bill via Telegram that directives may be stale.
2. Read `~/aadp/claudis/DIRECTIVES.md` — follow any standing instructions before proceeding.
3. Read `~/aadp/claudis/CONTEXT.md` — system facts and operational context.
4. Read `~/aadp/claudis/TRAJECTORY.md` — destinations, active vectors, operational state.
5. Ready for directive.
