...
| context_engineering_research | — | On-demand research agent. Searches HN Algolia + arXiv for 5 hardcoded context engineering queries, fetches pages, summarizes top results with Haiku, writes to research_articles table (dedup by URL). Logic delegated to stats_server /run_context_research (no credentials in n8n JSON). Error logging to error_logs. Webhook: POST /webhook/context-engineering-research. Workflow: gzCSocUFNxTGIzSD. **Built 2026-04-26 (B-055).** |
