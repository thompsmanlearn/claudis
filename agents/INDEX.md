...
| agent_health_monitor | — | Fleet-wide automated health monitoring. Scans all active agents for consecutive n8n execution errors. Flags agents with ≥1 consecutive error, auto-pauses agents at ≥3 (PATCH status=paused in agent_registry + sandbox_notify alert). Writes full scan report (scan_time, total_agents_checked, agents_with_issues, summary, issues[], retiring[]) to experimental_outputs + audit_log on every run. Webhook: POST /webhook/agent-health-monitor. Workflow: w5vypq4vb2rSrwdl. **Promoted 2026-04-12** (autonomous, 5-criteria passed: 1 verified run exec-2250, webhook-only V2 compliant, $0/month, no destructive SQL, Bill notified). Complements behavioral_health_check (per-agent deep analysis) with fleet-level triage. |
...
---
*Last updated: 2026-04-12 (Sentinel session) — agent_health_monitor promoted to production (webhook-only, fleet scan, auto-retire at 3 consecutive errors, Bill notified). First live scan: 16 agents, telegram_command_agent flagged with 1 consecutive error (last run April 2).*
